        ...
        if status["state"] == "running":
            return create_embed_response(
                ...
                fields=[
                    {
                        "name": "IP",
                        "value": f"`{status['public_ip']}:{MINECRAFT_PORT}`",
                        "inline": True,
                    },
...
def handle_stop(options: list) -> dict:
    ...
    try:
        ...
        if players["player_count"] > 0:
            player_list = (
                ", ".join(players["players"]) if players["players"] else "不明"
            )
            return create_embed_response(
                title="==プレイヤーがいます==",
        ...
        return create_embed_response(
            ...
            color=Colors.RED,
            fields=[{"name": "状態", "value": "🔴 Stopped", "inline": True}],
        )
...
def handle_status(options: list) -> dict:
    ...
    try:
        ...
        fields = [
            {
                "name": "EC2 状態",
                "value": f"{state_icon} {status['state']}",
                "inline": True,
            },
            {
                "name": "インスタンスタイプ",
                "value": status["instance_type"],
                "inline": True,
            },
        ]
        ...
        if status["state"] == "running":
            ...
            if players["online"]:
                ...
                fields.append(
                    {"name": "プレイヤー", "value": player_info, "inline": False}
                )
                mc_status = "🟢 オンライン"
            else:
                mc_status = "🟡 起動中/準備中"
        ...
        if result["success"]:
            return create_embed_response(
                ...
                fields=[
                    {
                        "name": "ファイル",
                        "value": result.get("backup_file", "N/A"),
                        "inline": False,
                    }
                ],
            )
